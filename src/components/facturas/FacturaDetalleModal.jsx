              ...
              <>
                {detalle.factura?.anulada && (
                  <div className="alert alert-danger mb-3">
                    ...
                    <div className="small mt-1">
                      ...
                      <br />
                      Fecha de anulación:{" "}
                      {detalle.factura.fecha_anulacion
                        ? new Date(
                            detalle.factura.fecha_anulacion,
                ...
                <div className="row g-3 mb-4">
                  {[
                    {
                      label: "Fecha",
                      val: new Date(detalle.factura?.fecha_hora).toLocaleString(
                        "es-CO",
                      ),
                    },
                    {
                      label: "Empleado",
                      val: detalle.factura?.empleado_nombres || "N/A",
                    },
                    {
                      label: "Cliente",
                      val: detalle.factura?.observaciones || "—",
                    },
                    {
                      label: "Total",
                      val: money(detalle.factura?.total_neto),
                      bold: true,
                    },
                  ].map((s) => (
                    <div className="col-6" key={s.label}>
                      ...
                      <div
                        className={
                          s.bold ? "fw-bold text-success fs-5" : "fw-semibold"
                        }
                      >
                        {s.val}
                      </div>
                      ...
                      {(detalle.productos ?? []).map((p, i) => (
                        <tr key={i}>
                          ...
                          <td className="text-end">
                            {money(p.precio_unitario_venta)}
                          </td>
                          <td className="text-end fw-semibold">
                            {money(p.subtotal_linea)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                      ...
                      {Number(detalle.factura?.total_iva) > 0 && (
                        <>
                          <tr>
                            ...
                            <td className="text-end">
                              {money(detalle.factura?.total_base)}
                            </td>
                          </tr>
                          <tr>
                            <td colSpan={3} className="text-end text-muted">
                            ...
                            <td className="text-end">
                              {money(detalle.factura?.total_iva)}
                            </td>
                          </tr>
                        </>
                      )}
