...
export default function PanelPago({
  ...
  return (
    <div className="px-4 py-3">
      ...
      <div className="mb-3">
        ...
        <div className="d-flex gap-2 flex-wrap">
          {metodosPago.map((m) => (
            ...
            >
              {m.nombre_metodo === "Efectivo"
                ? "💵"
                : m.nombre_metodo === "Transferencia"
                  ? "📱"
                  : "💳"}{" "}
              {m.nombre_metodo}
            </button>
      ...
      {metodoPago && !esEfectivo && (
        ...
        >
          ...
          <div className="small text-muted mt-1">
            El monto se registra automáticamente
          </div>
        </div>
      )}
      ...
      {esEfectivo && pago > 0 && (
        <div
          className={`d-flex justify-content-between align-items-center rounded p-2 mb-3 ${cambio >= 0 ? "bg-success bg-opacity-10" : "bg-danger bg-opacity-10"}`}
        >
          <span className="small fw-semibold">🪙 Cambio</span>
          <span
            className={`fw-bold ${cambio >= 0 ? "text-success" : "text-danger"}`}
          >
            {money(cambio)}
          </span>
        </div>
      ...
      <button
        ...
        disabled={
          cobrandoLoad || !puedeCobrar({ items, metodoPago, pago, total })
        }
        onClick={onCobrar}
      >
        {cobrandoLoad ? (
      ...
      {hayItems && !metodoPago && (
        <p className="text-warning small text-center mt-2 mb-0">
          ⚠️ Selecciona método de pago
        </p>
      )}
      {hayItems && esEfectivo && pago > 0 && pago < total && (
        <p className="text-danger small text-center mt-2 mb-0">
          ⚠️ Faltan {money(total - pago)} para completar el pago
        </p>
      )}
