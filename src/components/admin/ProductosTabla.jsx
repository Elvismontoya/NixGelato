...
export default function ProductosTabla({
  q, setQ, filterCat, setFilterCat, sortBy, setSortBy, onlyToppings, setOnlyToppings,
  categorias, loading, productos, msgTabla,
  onLimpiarFiltros, onHistorial, onEditar, onEliminar,
}) {
  return (
        ...
        <div className="card-body">
          ...
          <div className="row g-2 align-items-end mb-3">
            <div className="col-12 col-md-4">
              ...
              <input
                className="form-control" placeholder="Nombre, categoría o ID…"
            ...
            <div className="col-12 col-md-2 d-flex gap-2">
              <div className="form-check mt-auto">
                <input
                  className="form-check-input" type="checkbox" id="onlyToppings"
                  checked={onlyToppings} onChange={(e) => setOnlyToppings(e.target.checked)}
                />
                <label className="form-check-label" htmlFor="onlyToppings">Solo toppings</label>
              </div>
            </div>
          </div>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="text-center py-5">
                      ...
                      <button className="btn btn-outline-brand" onClick={onLimpiarFiltros}>
                        Limpiar filtros
                      </button>
                    </td>
                  </tr>
                  ...
                  productos.map((p) => (
                    <tr key={p.id}>
                      <td>
                        <div className="d-flex align-items-center gap-2">
                          {p.img ? (
                            ...
                            <div style={{
                              width: 48, height: 48, borderRadius: 8, border: "1px dashed #ddd",
                              display: "flex", alignItems: "center", justifyContent: "center",
                              fontSize: 10, color: "#888", backgroundColor: "#f8f9fa",
                            }}>
                              sin img
                            </div>
                          )}
                      ...
                      <td className="text-center">
                        <span className="badge bg-light text-dark">{p.categoria || "Sin categoría"}</span>
                      </td>
                      <td className="text-center"><span className="price-badge">{money(p.precio)}</span></td>
                      <td className="text-center">
                        <span className={`badge ${badgeStock(p.stock)}`}>{p.stock}</span>
                      </td>
                      <td className="text-center">
                        {p.permiteToppings
                      ...
                      <td className="text-end">
                        <div className="d-flex justify-content-end gap-2 flex-nowrap">
                          <button type="button" className="btn btn-sm btn-outline-secondary btn-table-action" onClick={() => onHistorial(p)} title="Historial de ventas">
                            📊
                          </button>
                          <button type="button" className="btn btn-sm btn-outline-brand btn-table-action" onClick={() => onEditar(p)}>
                            Editar
                          </button>
                          <button type="button" className="btn btn-sm btn-outline-danger btn-table-action" onClick={() => onEliminar(p)}>
                            Eliminar
                          </button>
                        </div>
                      </td>
