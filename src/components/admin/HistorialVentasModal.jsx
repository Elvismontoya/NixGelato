...
export default function HistorialVentasModal({ modal, data, loading, onClose }) {
        ...
        <div className="card border-0 shadow-lg" style={{ width: "100%", maxWidth: 600, borderRadius: "1.25rem", pointerEvents: "all", maxHeight: "85vh", overflowY: "auto" }}>
          <div className="card-header border-0 pt-4 pb-3 px-4" style={{ background: "linear-gradient(135deg, var(--sky, #6cd2f7), var(--aqua, #91eed3))" }}>
            <div className="d-flex justify-content-between align-items-center">
              <h5 className="fw-bold mb-0">📊 Historial de ventas — {modal.nombre}</h5>
            ...
            {loading ? (
              ...
              <>
                <div className="row g-3 mb-4">
                  {[
                    { label: "Unidades vendidas", val: data?.resumen?.total_unidades ?? 0 },
                    { label: "Ventas registradas", val: data?.resumen?.total_ventas ?? 0 },
                    { label: "Ingresos generados", val: money(data?.resumen?.total_ingresos ?? 0) },
                  ].map((s) => (
                      ...
                      <tbody>
                        {data.por_dia.map((d) => (
                          <tr key={d.fecha}>
                            ...
                            <td className="text-center"><span className="badge bg-info">{d.unidades}</span></td>
