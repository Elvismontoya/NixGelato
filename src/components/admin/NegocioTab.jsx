...
export default function NegocioTab() {
  ...
  useEffect(() => {
    getConfig()
      ...
      .catch(() => setMsg({ text: "No se pudo cargar la configuración", type: "danger" }))
  ...
  async function onSubmit(e) {
    ...
    try {
      const body = Object.fromEntries(CAMPOS.map((c) => [c.name, form[c.name]]));
  ...
  if (!form) {
    return <div className="alert alert-danger">{msg.text || "Sin configuración"}</div>;
  }
  ...
  return (
    <div className="row g-4 fade-in">
        ...
        <div className="card card-soft h-100">
          <div className="card-body">
            ...
            <p className="text-muted small mb-3">
              Se usan en el recibo térmico (documento equivalente). El IVA se aplica al desglose del ticket.
            </p>
              ...
              {CAMPOS.map((c) => (
                <div className="mb-3" key={c.name}>
                  ...
                  <input
                    ...
                    value={form[c.name] ?? ""}
                    onChange={onChange}
                    {...(c.type === "number" ? { min: 0, max: 100, step: "0.5" } : {})}
                  />
                </div>
              ))}
      ...
      <div className="col-lg-6">
        <div className="card card-soft h-100">
          <div className="card-body">
            <h6 className="fw-bold mb-3">Vista previa de la cabecera del ticket</h6>
            <div className="p-3 rounded text-center" style={{ background: "var(--bg-soft, #f8f9fa)", fontFamily: "monospace" }}>
              ...
              {form.telefono && <div className="small">Tel: {form.telefono}</div>}
              {form.regimen && <div className="small fw-semibold">{form.regimen}</div>}
              <hr />
