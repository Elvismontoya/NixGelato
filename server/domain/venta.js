...
const NEGOCIO =
  /^(SIN_PRODUCTOS|ITEM_INVALIDO|METODO_PAGO_INVALIDO|PRODUCTO_NO_DISPONIBLE|SIN_INVENTARIO|STOCK_INSUFICIENTE)/;
...
export function traducirErrorVenta(pgMessage = "") {
  if (pgMessage.startsWith("SIN_PRODUCTOS"))
    return "No hay productos en la venta.";
  if (pgMessage.startsWith("ITEM_INVALIDO"))
    return "Hay productos con datos inválidos.";
  if (pgMessage.startsWith("METODO_PAGO_INVALIDO"))
    return "El método de pago seleccionado no es válido.";
  if (pgMessage.startsWith("PRODUCTO_NO_DISPONIBLE"))
    return "Uno de los productos ya no está disponible.";
  if (pgMessage.startsWith("SIN_INVENTARIO"))
    return "Uno de los productos no tiene inventario configurado.";
  if (pgMessage.startsWith("STOCK_INSUFICIENTE")) {
    const m = pgMessage.match(
      /STOCK_INSUFICIENTE:(.+?) disp (\d+) pedido (\d+)/,
    );
    return m
