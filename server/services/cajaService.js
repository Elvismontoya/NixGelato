...
export function crearCajaService({ cajaRepo, auditoriaRepo, clock }) {
  ...
  async function abrir({ monto_apertura, observaciones }, actorId) {
    ...
    auditoriaRepo.log({
      idEmpleado: actorId, accion: 'INSERT', tabla: 'aperturas_caja',
  ...
  async function cerrar({ id_apertura, monto_cierre, observaciones }, actorId) {
    ...
    if (apertura.estado === 'cerrada') throw ApiError.badRequest('Esta caja ya está cerrada')
    ...
    if (!cierreFinal) throw ApiError.conflict('Esta caja ya fue cerrada por otro usuario')

    auditoriaRepo.log({
      idEmpleado: actorId, accion: 'UPDATE', tabla: 'aperturas_caja', idRegistro: id_apertura,
