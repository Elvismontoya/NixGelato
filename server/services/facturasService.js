...
import {
  ...
  traducirErrorVenta,
  normalizarItems,
} from "../domain/venta.js";

function mapFacturaResumen(f) {
  return {
    id_factura: f.id_factura,
    fecha_hora: f.fecha_hora,
    empleado_nombres: f.empleados
      ? `${f.empleados.nombres} ${f.empleados.apellidos}`
      : null,
...
export function crearFacturasService({
  facturasRepo,
  metodosPagoRepo,
  auditoriaRepo,
}) {
  async function registrarVenta(body, actorId) {
    ...
    if (!metodo_pago)
      throw ApiError.badRequest("Debe seleccionar un método de pago.");
  ...
  async function ingresosPorDia({ fecha_desde, fecha_hasta }) {
    const facturas = await facturasRepo.ingresosPorDia({
    ...
    for (const f of facturas) {
      ...
      if (!porDia[fecha])
        porDia[fecha] = { fecha, ingresos_totales: 0, total_ventas: 0 };
    ...
    return Object.values(porDia)
      .map((it) => ({
        ...
        promedio_venta:
          it.total_ventas > 0 ? it.ingresos_totales / it.total_ventas : 0,
      }))
  ...
  async function anular(id, motivo, actorId) {
    if (!motivo?.trim()) {
      throw ApiError.badRequest(
        "Debes indicar un motivo para anular la venta.",
      );
    }
    ...
    try {
      ...
      data = await facturasRepo.anularVenta({
        idFactura: id,
        motivo,
        idEmpleado: actorId,
      });
      ...
      const m = err.message || "";
      if (m.startsWith("FACTURA_NO_EXISTE"))
        throw ApiError.notFound("Factura no encontrada");
      if (m.startsWith("YA_ANULADA"))
        throw ApiError.badRequest("Esta venta ya fue anulada");
      if (m.startsWith("MOTIVO_REQUERIDO"))
        throw ApiError.badRequest(
          "Debes indicar un motivo para anular la venta.",
        );
      throw err;
