...
function normalizarTarifaIva(valor) {
  ...
  if (!Number.isFinite(n) || n < 0 || n > 100) {
    throw ApiError.badRequest(
      "El IVA del producto debe ser un porcentaje entre 0 y 100",
    );
  }
  return n;
}

export function crearProductosService({
  productosRepo,
  categoriasRepo,
  inventarioRepo,
  auditoriaRepo,
  ...
  async function ventas(id, { fecha_desde, fecha_hasta }) {
    ...
    if (fecha_desde)
      filas = filas.filter(
        (v) => v.facturas?.fecha_hora >= `${fecha_desde}T00:00:00`,
      );
    if (fecha_hasta)
      filas = filas.filter(
        (v) => v.facturas?.fecha_hora <= `${fecha_hasta}T23:59:59`,
      );
    ...
    for (const v of filas) {
      ...
      if (!porDia[fecha])
        porDia[fecha] = { fecha, unidades: 0, ingresos: 0, ventas: 0 };
    ...
    return {
      ...
      por_dia: Object.values(porDia).sort(
