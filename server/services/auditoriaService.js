...
export function crearAuditoriaService({ auditoriaRepo, facturasRepo, clock }) {
  async function listar(rawLimit) {
    ...
    return filas.map((it) => ({
      ...
      empleado:             it.empleados ? `${it.empleados.nombres} ${it.empleados.apellidos}` : 'Sistema',
      accion:               it.accion,
  ...
  async function registrar(body, actorId) {
    ...
    if (!accion?.trim()) throw ApiError.badRequest('El campo accion es requerido')
  ...
  async function ingresosHoy() {
    ...
    const ingresos = facturas.reduce((t, f) => t + (Number(f.total_neto) || 0), 0)
