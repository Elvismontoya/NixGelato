...
export function crearAuthService({ empleadosRepo, rolesRepo, authRepo, hasher, tokenSigner }) {
  async function needsInitialAdmin() {
    const empleados = await empleadosRepo.listActiveWithRole()
    const hayAdmin = empleados.some((e) => e.roles?.nombre_rol?.toLowerCase() === 'admin')
    return !hayAdmin
  }
  ...
  async function login({ usuario, password }) {
    ...
    const rol = (await rolesRepo.findNameById(emp.id_rol))?.toLowerCase() || 'cajero'
    return {
      token: tokenSigner.sign({ id_empleado: emp.id_empleado, usuario_login: emp.usuario_login, rol }),
      rol,
    }
  }
  ...
  async function registerAdmin({ nombres, apellidos, usuario, password }) {
    ...
    try {
      ...
      if (msg.startsWith('ADMIN_YA_EXISTE'))   throw ApiError.forbidden('Ya existe un administrador registrado.')
      if (msg.startsWith('USUARIO_EN_USO'))    throw ApiError.badRequest('El nombre de usuario ya está en uso.')
      if (msg.startsWith('USUARIO_REQUERIDO')) throw ApiError.badRequest('El usuario es obligatorio.')
      throw err
    }

    return {
      token: tokenSigner.sign({ id_empleado: data.id_empleado, usuario_login: data.usuario_login, rol: 'admin' }),
      rol: 'admin',
    }
  }
