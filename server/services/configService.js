...
export function crearConfigService({ configRepo, auditoriaRepo }) {
  ...
  async function actualizar(body, actorId) {
    ...
    for (const k of CAMPOS) {
      ...
      fields[k] =
        k === "iva_porcentaje" ? Number(body[k]) : String(body[k]).trim();
    }
    ...
    if (
      "iva_porcentaje" in fields &&
      (!Number.isFinite(fields.iva_porcentaje) ||
        fields.iva_porcentaje < 0 ||
        fields.iva_porcentaje > 100)
    ) {
