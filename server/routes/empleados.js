...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const { id_empleado } = await empleadosService.crear(
      req.body ?? {},
      req.user.id_empleado,
    );
    res
      .status(201)
      .json({ message: "Empleado creado correctamente", id_empleado });
  }),
);
...
router.put(
  "/:id",
  validate(actualizarEmpleadoSchema),
  asyncHandler(async (req, res) => {
    await empleadosService.actualizar(
      req.params.id,
      req.body ?? {},
      req.user.id_empleado,
    );
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    await empleadosService.cambiarPassword(
      req.params.id,
      req.body?.password,
      req.user.id_empleado,
    );
