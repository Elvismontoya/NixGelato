...
// POST /api/categorias
router.post('/', verifyToken, requireAdmin, validate(categoriaSchema), asyncHandler(async (req, res) => {
...
router.put('/:id', verifyToken, requireAdmin, validate(categoriaSchema), asyncHandler(async (req, res) => {
...
router.delete('/:id', verifyToken, requireAdmin, asyncHandler(async (req, res) => {
  const { message, categoria } = await categoriasService.eliminar(req.params.id, req.user.id_empleado)
